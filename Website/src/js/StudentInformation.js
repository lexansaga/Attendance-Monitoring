...
let subject = url.searchParams.get('sub')
let subtitle = url.searchParams.get('title')
...
$(document).ready(function () {
  ...
  firebase.database().ref(`Data/Student/Information/${id}`).once('value', snap => {
    ...
    let Department = snap.child('Department').val()
    ...
    $('.statuslabel').text(subject == null ? 'Overall Status' : `Status on ${subtitle}`)
    $('.id').text(id)
    ...
    CountStatus($('.tleaveearly'), 'leaveearly')
...
function CountStatus(object, aStatus) {
  if (subject != null) {
    console.log(subject)
    //For individual subject
    var sCount = 0;
    firebase.database().ref(`Attendance/Summary/Class/${subject}/`).once('value', snap => {
      let dates = snap.child('Dates')
      dates.forEach(date => {
        let students = date.child('Student')
    ...
    console.log('Overall')
    var sCount = 0;
    firebase.database().ref(`Attendance/Summary/Class/`).once('value', snap => {
      snap.forEach(Class => {
